...
/*
itemsToDisplay is an array of the API items to display in the widget. The available items are: 'todayCases', 'todayDeaths', 'recovered', 'active', 'deaths', and 'cases'. The items will appear in the order they are listed in the array.

The default array is: ['cases','deaths','active','recovered']
*/
let itemsToDisplay = ['cases','todayCases','deaths','active']
/*--------------------------
...
v1.5 beta 1 - Add ability to choose API items to be displayed by setting up the itemsToDisplay array
--------------------------*/
...
if (itemsToDisplay.length > 4) throw new Error("The maximum number of items has been exceeded. Four items is the max allowed by the widget. Please reduce the itemsToDisplay array to four items or less")

let widget = new ListWidget()

const country = args.widgetParameter?args.widgetParameter:'USA'//'Guatemala'
...
itemsToDisplay.forEach((item) => {
  addItem(item,checkForValueInJSON(item))
})
/*
...
function addItem(item,itemValue){
  ...
  let trendColors = {'recovered':{up:Color.green().hex,down:Color.red().hex},'cases':{up:Color.red().hex,down:Color.green().hex},'todayDeaths':{up:Color.red().hex,down:Color.green().hex},'todayCases':{up:Color.red().hex,down:Color.green().hex},'deaths':{up:Color.red().hex,down:Color.green().hex},'active':{up:Color.red().hex,down:Color.green().hex}}
